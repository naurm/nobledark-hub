...
export default function AdminPage() {
  ...
  const save = async (updated: BookEntry[]) => {
    setSaving(true)
    setMessage("")
    const res = await fetch("/api/books", {
      method: "PUT",
  ...
  const removeBook = (slug: string) => {
    ...
    save(books.filter((b) => b.slug !== slug))
  }
      ...
      <section className="mx-auto max-w-5xl px-6 pt-8 pb-24">
        ...
        <div className="space-y-2">
          {books.map((book, i) => (
            <div
              key={book.slug}
              className="rounded-lg border border-stone-800 bg-stone-900/50 p-4 hover:border-stone-700 transition-colors"
            >
              {editingSlug === book.slug ? (
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 rounded bg-stone-800 border border-stone-700 text-sm mt-1"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="text-[10px] font-mono uppercase tracking-wider text-stone-600">Tags (comma-separated)</label>
                    <input
                  ...
                  <div>
                    ...
                    <textarea
                      value={editForm.whyNobledark || ""}
                      onChange={(e) => setEditForm({ ...editForm, whyNobledark: e.target.value })}
                      className="w-full px-3 py-1.5 rounded bg-stone-800 border border-stone-700 text-sm mt-1 h-20 resize-none"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      onClick={saveEdit}
                      className="px-4 py-1.5 rounded bg-amber-700 hover:bg-amber-600 text-stone-100 text-xs transition-colors"
                    >
                      Save
                    </button>
                    <button
                      onClick={() => setEditingSlug(null)}
                      className="px-4 py-1.5 rounded border border-stone-700 hover:border-stone-500 text-stone-400 text-xs transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <button
                      onClick={() => moveUp(i)}
                      disabled={i === 0}
                      className="text-stone-600 hover:text-stone-300 disabled:opacity-20 disabled:cursor-not-allowed"
                    >
                      ▲
                    </button>
                    <button
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      ...
                      className="px-3 py-1 rounded border border-stone-700 hover:border-stone-500 text-stone-400 text-xs transition-colors"
                    >
                      Edit
                    </button>
                    <button
